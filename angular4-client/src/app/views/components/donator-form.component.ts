...
export class DonatorFormComponent {
  ...
  buscaCidades(estados: string) {
    var num_estados = this.json_citys.estados.length;
    var j_index = -1;

    for(var x=0;x<num_estados;x++){
      if(this.json_citys.estados[x].sigla == estados){
         j_index = x;
      }
    }
    ...
    if(j_index != -1){
      ...
      this.citys = this.json_citys.estados[j_index].cidades;
    } else {
      this.citys = [];
    }
  }

  updateCitys(content: Event) {
    //console.log(content);
    this.buscaCidades(content.toString());
  }
